...
export class PaymentsService {
  ...
  async create(userId: string, input: CreatePaymentInput): Promise<SavingPayment> {
    ...
    const now = new Date(); const expiresAt = new Date(now.getTime() + plan.paymentExpiresInMinutes * 60000).toISOString(); const paymentId = randomUUID();
    ...
    payment.description = `TK${payment.orderCode}`;
    ...
    try {
      ...
      payment.status = "PENDING"; payment.paymentLinkId = provider.paymentLinkId; payment.checkoutUrl = provider.checkoutUrl; payment.qrCode = provider.qrCode;
      return payment;
    } catch (error) { payment.status = "FAILED"; payment.errorCode = "PAYMENT_CREATE_FAILED"; payment.errorMessage = "Không thể tạo mã thanh toán."; this.store.releaseSlot(payment); this.payos.logProviderFailure(error); throw new ApiError("PAYMENT_CREATE_FAILED", "Không thể tạo mã thanh toán.", 502); }
  }
  ...
  private expireIfNeeded(payment: SavingPayment): void { if (["CREATING", "PENDING", "PROCESSING"].includes(payment.status) && new Date(payment.expiresAt).getTime() <= Date.now()) { payment.status = "EXPIRED"; this.store.releaseSlot(payment); } }
}
