...
export class SavingPlansController {
  ...
  @Delete(":planId") async remove(@CurrentUser() user: PublicUser, @Param("planId") planId: string) { const deleted = await this.plans.remove(user.id, planId); return { success: true, data: { deletedPlanId: deleted.id } }; }
