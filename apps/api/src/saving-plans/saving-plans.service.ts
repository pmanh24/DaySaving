...
export class SavingPlansService {
  ...
  start(userId: string, planId: string) { const plan = this.store.getPlan(userId, planId); if (plan.status === "COMPLETED") throw new ApiError("PLAN_COMPLETED", "Kế hoạch đã hoàn thành.", 409); plan.status = "ACTIVE"; plan.activatedAt ??= new Date().toISOString(); return plan; }
