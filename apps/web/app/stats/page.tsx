...
async function loadStatistics(accessToken: string | null): Promise<Statistics> {
  if (isDemoMode) { const { challenge } = getDemoBoard(); return { totalSaved: challenge.savedAmount, targetAmount: challenge.targetAmount, progressPercent: challenge.progressPercent, completedCells: challenge.completedCells, remainingCells: challenge.totalCells - challenge.completedCells, averagePerDay: Math.round(challenge.savedAmount / Math.max(challenge.completedCells, 1)), currentStreak: challenge.streak }; }
  const board = await apiRequest<BoardResponse>("/challenges/current", {}, accessToken ?? undefined);
  return apiRequest<Statistics>(`/challenges/${board.challenge.id}/statistics`, {}, accessToken ?? undefined);
}

export default function StatsPage() {
  const { accessToken } = useAuth();
  const { data: stats } = useQuery({ queryKey: ["statistics", accessToken], queryFn: () => loadStatistics(accessToken), enabled: isDemoMode || Boolean(accessToken) });
  if (!stats) return <SimplePage active="stats" eyebrow="GÓC NHÌN CỦA BẠN" title="Thống kê" subtitle="Đang tải nhịp tiết kiệm…"><div className="empty">Đang tải dữ liệu.</div></SimplePage>;
